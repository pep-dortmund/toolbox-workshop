...
dortmund = 6.03609
rki = 5.88250
n_positive_tests = np.convolve(data['Zuwachs_positiver_Testergebnisse_zum_Vortag'], np.ones(7))

plt.bar(data['Datum'], data['Zuwachs_positiver_Testergebnisse_zum_Vortag'], label='Positive Fälle')

plt.plot(data['Datum'], n_positive_tests[:-6]/dortmund, 'k.', markersize=3, label='7-Tage Inzidenz Do')
plt.plot(data['Datum'], n_positive_tests[:-6]/rki, 'c.', markersize=3, label='7-Tage Inzidenz RKI')
...
plt.tight_layout(pad=0)
plt.savefig('loesung-inzidenz.pdf')
plt.clf()

plt.title('Gesamtzahl an Fällen')
plt.bar(data['Datum'], data['positive_Testergebnisse_insgesamt'].astype(int), label='positive Tests')
plt.bar(data['Datum'], data['genesene_Personen_gesamt'].astype(int), label='Genesene')
plt.bar(data['Datum'], 50 * data['ursächlich_an_COVID19_Verstorbene'].astype(int), label=r'Verstorbene $\cdot 50$')
plt.bar(data['Datum'], 50 * data['aufgrund_anderer_Ursachen_Verstorbene'].astype(int), label=r'an anderer Ursache Verstorbene $\cdot 50$')
plt.legend()
plt.xticks(data['Datum'][::14], data['Datum'][::14], rotation=90)
plt.xlim(-1, data['Datum'].size)
plt.ylim(0, np.max(data['positive_Testergebnisse_insgesamt'].astype(int))*1.05)
plt.tight_layout(pad=0)
...
plt.xticks(data['Datum'][::14], data['Datum'][::14], rotation=90)
plt.xlim(-1, data['Datum'].size)
plt.ylim(0, np.max(data['aktuell_erkrankte_Personen'].astype(int))*1.05)
plt.tight_layout(pad=0)
