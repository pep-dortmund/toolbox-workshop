...
N = np.delete(N, [0,1,2,3, len(N)-1, len(N)-2, len(N)-3, len(N)-4, len(N)-5])
N = N[::2]
...
ax.annotate("Messwerte einlesen: numpy", xy=(t[9],N[9]),
            xytext=(t[9]+0.5, N[9]+10))

ax.annotate("Fit: scipy", xy=(t[50],N[50]),
            xytext=(t[50]+0.5, N[50]+30), color='r')

ax.annotate("Fehlerrechnung: uncertainties", xy=(t[20],N[20]),
            xytext=(t[20]+0.5, N[20]+30), color='r', alpha=0.3)
