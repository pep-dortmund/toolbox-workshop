      ...
      \begin{itemize}
        ...
        \item spezielle Verzeichnisse:
          \begin{description}[\texttt{..}]
            ...
            \item[\texttt{-}] das vorherige Verzeichnis
          \end{description}
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{\texttt{chmod}}
  \begin{tabu}{>{\ttfamily}l X[,L]}
    ...
    chmod \textit{[options] UGO file1 ...}    & \enquote{change mode}: Verändert die klassischen Unix-Dateirechte \\
  \end{tabu}
    \begin{itemize}
        \item \texttt{0}: Keine,\item \texttt{1}: Nur Ausführen,\item \texttt{2}: Nur Schreiben,\item \texttt{3}: Schreiben und Ausführen,\item \texttt{4}: Nur Lesen,\item \texttt{5}: Lesen und Ausführen,\item \texttt{6}: Lesen und Schreiben,\item \texttt{7}: Voll 
    \end{itemize}
  \begin{tabu}{>{\ttfamily}l X[,L]}
    chmod 111 beispiel.txt    & Beispiel: Für alle nur noch ausführbar \\
  \end{tabu}
\end{frame}
